...
def transform_generic_json(entry, file_path):
    ...
    Только нормализует. Улучшено: сохранение ID и потерянных полей в args без дубликатов.
    """
    if not isinstance(entry, dict):
    ...
    # 12. Args - собираем оставшиеся поля + восстановленные потерянные (без дубликатов)
    args = {}
    try:
        # Оставшиеся поля (исключая стандартные + message/log.level для избежания дубликатов)
        excluded_remaining = standard_out | {"message", "log.level", "log", "process", "service", "host", "trace", "transaction", "logger", "component", "app_id", "cluster.name", "node.name"}
        for k, v in original.items():
            if k not in excluded_remaining:
                args[k] = v

        # Восстановление потерянных полей в args (без дублирующихся ключей)
        # Trace (полностью, но без id)
        if "trace" in original:
            trace_data = original["trace"].copy()
            trace_data.pop("id", None)
            if trace_data:  # Добавляем только если что-то осталось
                args["trace"] = trace_data

        # Transaction (полностью, но без id)
        if "transaction" in original:
            transaction_data = original["transaction"].copy()
            transaction_data.pop("id", None)
            if transaction_data:  # Добавляем только если что-то осталось
                args["transaction"] = transaction_data

        # Host.user (без name, если дублирует tn)
        if "host" in original and isinstance(original["host"], dict) and "user" in original["host"] and isinstance(original["host"]["user"], dict):
            user_data = original["host"]["user"].copy()
            user_data.pop("name", None)  # Если name дублирует tn
            if user_data:
                if "host" not in args:
                    args["host"] = {}
                args["host"]["user"] = user_data
            else:
                if "host" not in original or "name" not in original["host"]:
                    if "host" not in args:
                        args["host"] = {}
                    args["host"]["name"] = original["host"].get("name")

        # Log (с logger, original, level=full; без дубликатов)
        if "log" in original:
            log_data = original["log"].copy()
            log_data["level"] = full_level  # Полная версия
            log_data.pop("level", None) if "level" in log_data and log_data["level"] == full_level else None
            args["log"] = log_data

        # Process (с thread.id, name, executable; без pid)
        if "process" in original:
            process = original["process"].copy()
            process.pop("pid", None)  # Не дублируем pid
            args["process"] = process

        # Удаляем дубликаты времени из args + рекурсивная чистка вложенных
        args = remove_time_duplicates_from_args(args, transformed.get("t"))
        args = clean_nested_duplicates(args, transformed)
...
def clean_nested_duplicates(nested_dict, transformed_entry):
    ...
    for k, v in nested_dict.items():
        ...
        # Проверяем, является ли текущий ключ дубликатом уровня l, pid, mt, tn, v, trace_id, transaction_id
            ...
            continue
        elif k == "id" and "trace" in transformed_entry and transformed_entry["trace_id"] == v:
            continue
        elif k == "id" and "transaction" in transformed_entry and transformed_entry["transaction_id"] == v:
            continue
...
def process_file_and_sort_chunk(args):
    ...
    try:
        ...
        # Кандидаты: сначала chardet (если conf > 0.7), потом utf-8 приоритетно, затем cp1251
        encodings_to_try = []
        if chardet_encoding and confidence > 0.7:
            encodings_to_try.append(chardet_encoding)
        encodings_to_try.extend(['utf-8', 'utf-8-sig', 'cp1251'])
        ...
        if content_lines is None:
            # Последний шанс: читаем с errors='replace' на utf-8
            logging.warning(f"Fallback to utf-8 with errors='replace' for {file_path}")
...
class LogMergerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("logmrgr v031.8")
