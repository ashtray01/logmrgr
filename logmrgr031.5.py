...
def transform_generic_json(entry, file_path):
    ...
    standard_out = {"t", "pid", "tr", "l", "lg", "mt", "tn", "v", "args", "ex"}
    original = entry
    transformed = {}
    ...
    # 2. Уровень
    if "l" in original and original["l"]:
        level = str(original["l"]).title()
        level = level.replace("Information", "Info").replace("Warning", "Warn").replace("Critical", "Fatal")
        transformed["l"] = level
    else:
        # ... (логика извлечения уровня) ...
        level = None
        ...
        if level:
            ...
            level_title = level_title.replace("Information", "Info").replace("Warning", "Warn").replace("Critical", "Fatal")
            transformed["l"] = level_title
        else:
            transformed["l"] = "Info"
    ...
    # 4. Trace
    if "tr" in original and original["tr"]:
        transformed["tr"] = original["tr"]
    ...
    # 11. Args - собираем оставшиеся поля
    args = {}
    for k, v in original.items():
        if k in transformed or k in standard_out or k in ["log", "process", "service", "host", "trace", "transaction", "logger", "component", "app_id", "cluster.name", "node.name"]:
            continue
        args[k] = v
...
class LogMergerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("logmrgr v031.5")
